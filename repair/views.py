...
class RepairListView(LoginRequiredMixin,ListView):
    ...
    def get_queryset(self,*args,**kwargs):
        ...
        if query:
            ...
            queryset = Repair.objects.all().filter(repairStatus=1).order_by('-pub_date').reverse()[:20]
...
class RepairMeListView(LoginRequiredMixin,ListView):
    ...
    def get_queryset(self,*args,**kwargs):
        ...
        if query:
            ...
            print("no")
            queryset = Repair.objects.filter(repairRequester=self.request.user.id).order_by('-id')[:20]
        return queryset
    model = Repair
...
class RepairStaffListView(LoginRequiredMixin,ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context_data['staff'] = Repair.objects.filter(repairStaff__is_staff=1).filter(repairStatus__in=mylist)
        context_data['wait'] = Repair.objects.filter(repairStatus=1).count()
        context_data['recieve'] = Repair.objects.filter(repairStatus=2).count()
        context_data['work'] = Repair.objects.filter(repairStatus=3).count()
        context_data['done'] = Repair.objects.filter(repairStatus=4).count()
