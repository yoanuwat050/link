...
class RepairDetail(models.Model):
    ...
    repairEstimateTime = models.TimeField(default='00:09:42')
    pub_date = models.DateTimeField('date published')
...
class Repair(models.Model):
    ...
    repairDesc = models.TextField(max_length=500,default='-')
    ...
    repairSummary = models.CharField(max_length=500,default='-')
    ...
    def save(self, *args, **kwargs):
        if self.repairStatus.id == 1:
            ...
            t2 = self.repairCause
            send = line.send_msg(t1),line.send_msg(t2),line.send_msg('ตรวจสอบ'),line.send_msg('http://18.206.248.221:8000/repair/')
        elif self.repairStatus.id != 1:
            token = self.repairRequestToken 
            line = Line(token)
            t1 = " การขอรับบริการของคุณ ได้รับการ",self.repairStatus
            
            send = line.send_msg(t1),line.send_msg('ตรวจสอบ:'),line.send_msg('http://18.206.248.221:8000/repair/allRepair/')
